...
/**
 * Middleware to check if user is owner of agency
...
const isAgencyOwner = async (req, res, next) => {
  try {
    ...
    // If agency is not found, respond with 404 status and render 404 page
    if (!agency) {
    ...
    // If user is owner, proceed to next middleware
    next();
  } catch (error) {
    // Log any unexpected errors to console
    console.error(error);
